	...
	<view class="container">
		...
		<uni-popup ref="popup">
			<view class="popup-content">
				<uni-forms ref="form" :rules="rules" :modelValue="eventItem" >
					...
					<uni-forms-item label="老公"  class="formItem" v-if="eventItem.type == 'marry'" name="husband" required>
					...
					<uni-forms-item label="姓名"  class="formItem" v-if="eventItem.type == 'birthday'" name="birthday" required>
						<uni-easyinput placeholder="最多4个字" v-model="eventItem.birthday" maxlength="4"
							:clearable="false" />
					</uni-forms-item>
		...
		<view>
			...
			<uni-popup ref="alertDialog" type="dialog">
				<uni-popup-dialog type="warn" cancelText="取消" confirmText="确认" title="删除通知" content="您确认删除本条记录吗?" @confirm="deleteEvent"
					></uni-popup-dialog>
			</uni-popup>
		</view>
	</view>
...
<script>
	...
	export default {
		...
		async mounted() {
			this.eventCategoryList.splice(0, 0, ...await getEventCategoryList())
			this.getAllEvent()
		},
		methods: {
			showModePage(type) {
			...
			changeEventType(idx) {
				...
				if(this.eventItem.type == 'marry'){
					this.rules.birthday = this.rules.bf = this.rules.gf = null;
					...
					this.rules.birthday = this.rules.wife = this.rules.husband = null;
			...
			async submit() {
				this.$refs.form.validate().then(async data => {
					// this.eventItem['className'] = classNameMap[this.eventItem.type]
					// this.eventItem['name'] = eventTypeMap[this.eventItem.type]
